odoo.define("pos_folio.pos_folio",function (require) {
    ...
    var FolioPopupWidget = Popup.extend({
        ...
        take_folio : function(pos,callable){
            ...
            var domain = [['name', '=', input_value],['state','=','draft']];
            rpc.query({
                model: 'hotel.folio',
                method: 'get_folio_data',
                args: [[],{'input_value': input_value}],
            }).pipe(function(result) {
                if (result && result.length > 0) {
                    var folio_customer_list = [];
                    var folio_dict = {};
                    _.each(result,function(re){
                        folio_dict[re.id] = re;
                        folio_customer_list.push({label:re.partner_id[1]+' - '+ re.name, item:re.id })
                    });
                    self.gui.show_popup('selection',{
                        title: _t('Select Folio Customer'),
                        list: folio_customer_list,
                        confirm: function (selected_customer) {
                            var order = []
                            var current_order = self.pos.get_order();
                            var folio = folio_dict[selected_customer]
                            current_order.set_folio_number(folio.name);
                            current_order.set_folio_number_id(folio.id);
                            current_order.set_client(self.pos.db.get_partner_by_id(folio.partner_id[0]));
                    ...
                    self.gui.show_popup('alert',{
                        title:_t('Warning !'),
                        body: _t('Folio/Room Number Not Found!!!'),
                   });
                   return false;
                    //alert('Folio/Room Number Not Found!!!');
                }
            });
        },
        close:function(){
