...
class PosConfig(models.Model):
    ...
    display_send_to_kitchen = fields.Boolean(
        "Display Send To Kitchen Button",
        help=""" If Display Send To kitchen Button is
        true than pos shows a send to kitchen button.""",
        default=True)
    display_kitchen_receipt = fields.Boolean(
        "Display Kitchen Receipt Button",
        help=""" If Display Kitchen Receipt Button is true than
        pos shows a kitchen receipt button.""",
        default=True)
    display_customer_receipt = fields.Boolean(
        "Display Customer Receipt Button",
        help=""" If Display Customer Receipt Button is true than
    ...
    def check_is_pos_restaurant(self):
        ...
        ir_module_module_ids = ir_module_module_object.search([
            ('state', '=', 'installed'),
            ('name', '=', 'pos_restaurant')])
...
class PosOrderLine(models.Model):
    ...
    partner_id = fields.Char(
        ...
        string="Customer")
    parcel = fields.Char(
        related='order_id.parcel_name',
        string="Parcel")
    table = fields.Char(
        related='order_id.table_name',
        string="Table")
    order_line_state_id = fields.Many2one(
        'pos.order.line.state',
    ...
    def orderline_state_id(self, pids, order_id):
        if pids:
            line_ids = [
                line.id for line in self.env['pos.order'].browse(
                    order_id).lines]
            if pids in line_ids:
...
class PosOrder(models.Model):
        ...
        for order in self:
            ...
            if order.state != 'paid':
                for line in order.lines:
                    count = count + 1
                    if expand_all_product_ids == [] and \
                            expand_four_product_ids == [] and count <= 4:
                        str1 = line.product_id.name + \
                            "____" + \
                            str(line.qty) + \
                            "-" + str(line.id) + \
                            '-' + \
    ...
    order_line_state_id = fields.Many2one(
        'pos.order.line.state', "Order Line State")
    is_show = fields.Boolean(
        'Show All', compute='show_all_product', store=True)
    parcel_name = fields.Char(
        compute='get_parcel_name', string='Parcel Name', store=True)
    table_name = fields.Char(compute='get_table_name',
                             string='Table Name', store=True)
    product_details = fields.One2many(
        "pos.order.line", compute='product_line', string='Product Details')
    ...
    def _process_order(self, order, kitchen=False):
        ...
        if kitchen and not order.get('id', False):
            ...
            print ("LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL:::::::::::::::",order.get('folio_id', False))
            if order.get('folio_id', False):
                folio = folio_obj.browse(order.get('folio_id', False))
                service_list = []
                for o_line in order_id.lines:
                    values = {'order_id': order_id.id,
                              'name': o_line.product_id.name,
                              'product_id': o_line.product_id.id,
                              'product_uom': o_line.product_id.uom_id.id,
                              'product_uom_qty': o_line.qty,
                              'price_unit': o_line.price_unit,
                              'price_subtotal': o_line.price_subtotal}
                    service_list.append((0,0,values))
                folio.write({'service_lines':service_list})
                order_id.state = 'done'
        ...
        if kitchen and order.get('id', False):
            ...
            for line in line_data:
                if line[2].get('line_id') in line_ids:
                    line[2]['order_line_state_id'] = pos_line_object.browse(
                        line[2].get('line_id')).order_line_state_id.id
                    pos_line_object.browse(
                        line[2].get('line_id')).write(line[2])
                    if 'property_product' in line[2]:
                        process_line = partial(
                            self.env['pos.order.line']._order_line_fields)
                        product_pro_list = [
                            process_line(l)
                            for l in line[2]['property_product']
                        ] if line[2]['property_product'] else False
                        if product_pro_list:
                            for product_pro in product_pro_list:
                                order.get('lines').append(product_pro)
                    order.get('lines').remove(line)
            self.browse(order_id).write(self._order_fields(order))
            if order.get('folio_id', False):
                folio = folio_obj.browse(order.get('folio_id', False))
                service_list = []
                order_data = self.browse(order_id)
                for o_line in order_data.lines:
                    values = {'order_id': order_data.id,
                              'name': o_line.product_id.name,
                              'product_id': o_line.product_id.id,
                              'product_uom': o_line.product_id.uom_id.id,
                              'product_uom_qty': o_line.qty,
            ...
            if not float_is_zero(
                ...
                if not cash_journal:
                    ...
                    if not cash_journal_ids:
                        ...
                        # credit card
                        # an amount higher than total amount of the order and
                        # gets cash back
                        cash_journal_ids = [
    ...
    def create_from_ui(self, orders, kitchen=False):
        ...
        existing_order_ids = self.search(
            [('pos_reference', 'in', submitted_references)]).sudo().ids
        existing_orders = self.browse(
            existing_order_ids).read(['pos_reference'])
    ...
    def _confirm_orders(self):
        for session in self:
            ...
            for order in session.order_ids.filtered(
                    lambda o: o.state not in ['done', 'invoiced', 'cancel']):
...
class PosMakePayment(models.TransientModel):
    ...
    def check(self):
        ...
        data = self.read()[0]
        # this is probably a problem of osv_memory as it's not compatible with
        # normal OSV's
        ...
        if order.test_paid():
            ...
            self.env['bus.bus'].sendmany(
                [[(self._cr.dbname, 'pos.order'), {'order_id': order.id}]])
